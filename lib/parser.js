 ...
 */
function RobotsParser (url, after_parse, request) {
  this.entries = [];
  this.sitemaps = [];
  this.defaultEntry = '';
...
RobotsParser.prototype.read = function(after_parse) {
  ...
  self.request(this.url, function (err, resp, body) { 
    var resp;
    if(err) {
      resp = {};
      resp.statusCode = 500;
    ...
    if ( [401, 403].indexOf(resp.statusCode) > -1 ) {
      ...
      self.request(redirect_url, function(err, response) {
        if(!err) {
      ...
      var allchunks = resp.body.toString();
